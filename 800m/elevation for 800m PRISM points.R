...
# for climate NA extraction
Pts_wgs84 = spTransform(Pts, CRS("+proj=longlat +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0"))

xy =  Pts_wgs84@coords
nrow(xy)
xy <- xy[!is.na(Pts@data$sz),]
nrow(xy)

head(xy)
df_xy = data.frame(ID1 = 1:nrow(xy), ID2 = ".", lat = xy[,2], long = xy[,1], el = ".")
head(df_xy)
write.csv(df_xy, "~/Documents/GIS_Lib/_climate/ClimateNA/California/raster template and coords for climate NA extract based on 270m BCM/coords.csv")
